            ...
            if not is_authed:
                ...
                if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                    return {
                        ...
                        "error": False,
                        "url": "/"
                        + ("" if upload_type == "file" else upload_type + "/")
                        + random_name,
...
def api_edit_paste():
    ...
    paste_row = config.db.select("pastes", where={"id": form["id"]}, singular=True)
        ...
        if len(form["paste"]) > 65000:
            ...
            if paste_row and (user_id or is_anon):
                ...
                config.db.insert(
                    "revisions",
                    {
                        "pasteid": paste_row["id"] if not is_fork else paste_id,
                        "userid": user_id,
                        "commit": commit,
                        "message": form["commit"],
                        "paste": form["paste"],
