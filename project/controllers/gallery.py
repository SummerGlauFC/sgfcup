...
from project.functions import get_setting, get_dict
...
def gallery_view(user_key=None):
    ...
    if user_id:
        ...
        if get_user_setting('block.value') and get_user_setting('gallery_password.value'):
            ...
            hex_pass = hashlib.sha1(get_user_setting(
                'gallery_password.value').encode('utf-8')).hexdigest()
        ...
        if results and error == '':
            for row in results:
                ...
                if row['ext'] == 'paste':
                    ...
                    full_file_path = get_setting(
                        'directories.files') + row['shorturl'] + row['ext']
        ...
        tally = config.db.fetchone(
            'SELECT SUM(`size`) AS `total` FROM `files` WHERE ' + (sql_search if query else '') + '`userid` = %s', format_list)
        usage = None
...
def gallery_auth_do(user_key):
    ...
    response.set_cookie(f'auth+{functions.get_userid(user_key)}',
        ...
        if is_paste:
            ...
            try:
                os.remove(get_setting('directories.files') +
                          shorturl + row['ext'])
                messages.append(
                    f'Removed file "{original}" ({shorturl})')
            except OSError:
...
def gallery_delete_advanced():
    ...
    if parts:
        user = config.db.select('accounts',
                                where={'key': parts['key'],
